...
class DeepStreamOutput(nn.Module):
    ...
    def forward(self, x):
        ...
        scores, labels = torch.max(scores, dim=-1, keepdim=False)
        
        # num_dets: number of detections per batch
        num_dets = torch.full((scores.shape[0],), scores.shape[1], dtype=torch.int32, device=scores.device)
        
        return num_dets, boxes, scores, labels.to(torch.int32)
...
def add_metadata(model_onnx, license_str):
    """Add license and citation metadata to ONNX model."""
    metadata = {
        "license": license_str,
        "citation": """@misc{peng2024dfine,
...
def main(args):
    ...
    # Load model for metadata and optional simplification
    ...
    model_onnx = add_metadata(model_onnx, args.license)
    ...
    external_data_file = onnx_output_file + ".data"
...
def parse_args():
    ...
    parser.add_argument("-c", "--config", required=True, type=str, help="Input YAML (.yml) file path (required)")
    parser.add_argument("-s", "--size", nargs="+", type=int, default=[640], help="Inference size [H,W] (default [640])")
    parser.add_argument("--opset", type=int, default=18, help="ONNX opset version")
    parser.add_argument("--simplify", action="store_true", help="ONNX simplify model")
    parser.add_argument("--dynamic", action="store_true", help="Dynamic batch-size")
    parser.add_argument("--batch", type=int, default=1, help="Static batch-size")
    parser.add_argument("--license", type=str, default="", help="License metadata for the ONNX model")
